...
class Course(models.Model):
    ...
    #need to create a one2many relation with session
    ...
    def _onchange_total_price(self):
        if self.base_price < 0.0:
            raise UserError(_('Base Price can not be set as Negative.'))
    ...
    def _check_additional_fee(self):
      #  for record in self:
        if self.additional_fee < 10.00:
            raise ValidationError(_()'Addtional Fees cannot be less than 10.00: %s' % self.additional_fee))
